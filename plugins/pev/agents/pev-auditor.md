...
hooks:
  PreToolUse:
    ...
    - matcher: ""
      hooks:
        - type: command
          command: "bash ${CLAUDE_PLUGIN_ROOT}/hooks/pev-tool-gate.sh 65 cortex_update_section,cortex_write_doc,cortex_add_section,cortex_delete_link,cortex_update_doc_meta,cortex_mark_clean,cortex_purge_node,cortex_build,cortex_check"
          timeout: 5
  PostToolUse:
    - matcher: ""
      hooks:
        - type: command
          command: "bash ${CLAUDE_PLUGIN_ROOT}/hooks/pev-tool-counter.sh 40 55 65"
          timeout: 5
---
