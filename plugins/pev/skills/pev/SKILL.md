...
**Pre-flight: clean working tree.** Run `git status` before anything else. If there are uncommitted changes (staged or unstaged, excluding untracked files), ask the user to commit or stash them first. A dirty working tree causes merge conflicts when the Builder's worktree branch is merged back. Do NOT proceed with uncommitted changes.
...
Capture baseline SHA (`git rev-parse HEAD`). Create the cycle manifest (see ref: `manifest-creation`).

### 2. Plan (Architect)

Write pev-state.json (see ref: `state-file`). Dispatch `pev-architect` subagent (see ref: `dispatch-prompts`).
...
Create isolated worktree, copy cortex DB, set up poetry venv (see ref: `worktree-commands`). Write pev-state.json for Builder — include `worktree_path` (see ref: `state-file`).

### 5. Build

**Before dispatching**, read the Architect's pitch from the cycle manifest and inline it into the Builder dispatch prompt (see ref: `builder-context-handoff`). The Builder uses cortex tools to read source on demand from the cortex DB snapshot copied into the worktree.
...
Handle status codes:
- **DONE**: Write manifest to `builder.manifest` section of cycle doc. Proceed to Phase 6.
...
Handle status codes:
- **PASS**: Write review to cycle doc. Proceed to Phase 7.
- **PASS_WITH_CONCERNS**: Write review to cycle doc. Present concerns to user. Options: (1) proceed to verify, (2) redispatch Builder to fix concerns, then re-review.
...
Merge worktree branch, remove worktree/branch, rebuild cortex, run cortex_check (see ref: `merge-commands`).

Construct change-set from `git diff` + Builder manifest. Write change-set to cycle doc. Update status to `auditor` (see ref: `status-updates`). (Builder manifest and review verdict are already in the cycle doc from earlier phases.)
...
- **Rejected**: Discuss options — revert merge and redispatch Builder with feedback.
...
Write pev-state.json for Auditor (see ref: `state-file`). Dispatch `pev-auditor` subagent on the main repo (see ref: `dispatch-prompts`).

Parse return — extract report from `---IMPACT-REPORT---` separator (see ref: `manifest-parsing`).

Handle status codes:
- **DONE**: Write Impact Report to `auditor.impact-report` section. The change ledger is already in `auditor.change-ledger` (written by Auditor as it works). Proceed to Phase 9.
...
**HUMAN GATE** — Present final audit results (nodes reviewed, change ledger entries, links, decisions, concerns). "Approve to commit and complete this PEV cycle?"
...
Clean up state file. Invoke `superpowers:finishing-a-development-branch`. Do NOT invoke `superpowers:requesting-code-review` — the PEV Reviewer (Phase 6) already covered spec compliance, functionality preservation, and code quality.
